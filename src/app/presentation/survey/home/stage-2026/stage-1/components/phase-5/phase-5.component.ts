...
@Component({
  ...
  styleUrl: './phase-5.component.css',
})
export class Phase5Component {
  ...
  private async validateAndSaveCurrentViewChild(
    ...
    if (this.tareaImplementada) {
      if (!isValidInfoImplem?.nombreImplementacion) {
        Swal.fire({
          html: 'No se puede continuar sin completar todos los campos requeridos en la información de la implementación,<span style="color:red;font-weight:bold"> falta ingresar nombre de la implementación</span>',
          icon: 'warning',
        });
        return null;
      }
  ...
  private async saveSurveyData(temporalData: any): Promise<boolean> {
    ...
    const objLog = {
      ...
      NOMBRE_IMPLEMENTACION:
        this.tareaImplementada ? temporalDataInformationImplementation.nombreImplementacion : '',
    ...
    let esTareaNoImplementada =
      temporalDataInformationImplementation?.esTareaNoImplementada || false;
            ...
            stage.survey.map((survey: any) => {
              if (survey.survey == `${this.stage}-${this.phase}`) {
                ...
                const nextSurvey = stages
                  ...
                  ?.survey.find(
                    (survey: any) =>
                      survey.survey == `${this.stage}-${this.phase + 1}`
                  );
