...
objQuestion: any = null;
  ...
  ngOnInit() {
    ...
    this._TemporalSaveService.dataNoTarea$.subscribe(data => {
      ...
      this.question = data ? 1 : 4;
    });
  }
  showComments(): boolean {
    return this.question == 4 && this.tareaImplementada;
  }
  ...
  private async saveSurveyData(temporalData: any): Promise<boolean> {
    ...
    const resolveStageYear = (stage: any) => stage?.YEAR ?? stage?.year ?? targetYear;
    const currentStageConfig = Array.isArray(stages)
      ? stages.find(
          (stage: any) => stage.stage == this.stage && resolveStageYear(stage) === targetYear
        )
      : null;
    ...
    const stageYear = resolveStageYear(currentStageConfig);
    ...
    /*
    console.log('[Stage-2026][Phase-1] Payload to insertar', {
      ...objLog,
    ...
    try {
      ...
      if (result) {
        stages.map((stage: any) => {
          if (stage.stage == this.stage && resolveStageYear(stage) === stageYear) {
  ...
  async getDataSaved() {
    ...
    try {
      ...
      const currentStage = stages.find(
        (s: any) => s.stage === this.stage && resolveStageYear(s) === targetYear
      );
      if (!currentStage) return;

      const currentSurvey_2026 = currentStage.survey.find((survey: any) => survey.survey === `${this.stage}-${this.phase}`);
      if (!currentSurvey_2026) return;

      const apiStage = resolveApiStage(this.stage, targetYear);
      ...
      dataResp = {
        ...dataResp,
        ARCHIVO_ACTA_NAME: currentSurvey_2026.dataSaved?.ARCHIVO_ACTA ?? '',
        ARCHIVO_EVIDENCIA_NAME: currentSurvey_2026.dataSaved?.ARCHIVO_EVIDENCIA ?? ''
      }
  ...
  fillQuestionAnswer(data: any, respuesta: any) {
    ...
    this.question = data.REGISTRA_TAREA ? 1 : 4;
