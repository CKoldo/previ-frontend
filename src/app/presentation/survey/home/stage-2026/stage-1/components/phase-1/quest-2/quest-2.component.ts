...
export class Quest2Component {
  ...
  getTotalDocentes(): number {
    return this.docentes.reduce(
      (sum, docente) => sum + (docente.cantidad * 1 || 0),
      0,
    );
  }

  getTotalGeneros(): number {
    return this.generos.reduce(
      (sum, genero) => sum + (genero.cantidad * 1 || 0),
      0,
    );
  }

  getTotalEdades(): number {
    return this.edades.reduce((sum, edad) => sum + (edad.cantidad * 1 || 0), 0);
  }
