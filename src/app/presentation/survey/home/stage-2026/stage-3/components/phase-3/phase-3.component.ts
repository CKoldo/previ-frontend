...
import { Quest4Component } from './quest-4/quest-4.component';
...
@Component({
  ...
  providers: [
    ...
    {
      provide: UpdateConfigurationCommand,
      useClass: UpdateConfigurationHandler,
    },
  ...
  styleUrls: ['./phase-3.component.css'],
})
export class Phase3Component {
  ...
  readonly phase = 3;
  readonly stage = 3;
  //readonly year: '2026' = '2026';
  @ViewChild(Quest1Component) quest1Component!: Quest1Component;
  @ViewChild(Quest2Component) quest2Component!: Quest2Component;
  @ViewChild(Quest3Component) quest3Component!: Quest3Component;
  @ViewChild(Quest4Component) quest4Component!: Quest4Component;
  ...
  ngOnInit() {
    ...
    this._TemporalSaveService.dataNoTarea$.subscribe((data) => {
      ...
      this.question = data ? 1 : 5;
    });
  }

  showComments(): boolean {
    return this.question == 5 && this.tareaImplementada;
  }
  ...
  async nextStep(step: number) {
    const stagePhaseMap: { [key: string]: any } = {
      ...
      3: this.quest3Component,
      4: this.quest4Component,
    };
  ...
  private async validateAndSaveCurrentViewChild(
    step: number,
    currentViewChild: any,
  ): Promise<any> {
    if (!currentViewChild) {
      Swal.fire({
    ...
    const isValidInfoImplem =
      await this.validateDataInformationImplementation();

    // if (!isValidInfoImplem?.fechaModalidad || !isValidInfoImplem?.modalidad || !isValidInfoImplem?.fechaImplementacion) {
    ...
    if (
      isValidInfoImplem?.modalidad === null ||
      isValidInfoImplem?.modalidad === undefined
    ) {
      Swal.fire({
    ...
    // En esta fase no es necesario validar el nombre de la implementación
    ...
    const existingDataIndex = temporalData.findIndex(
      (item: any) => item.question === step,
    );
    if (existingDataIndex > -1) {
  ...
  private async saveSurveyData(temporalData: any): Promise<boolean> {
    ...
    const pickStageYear = (stageEntry: any) =>
      stageEntry?.YEAR ?? stageEntry?.year ?? targetYear;

    const currentStageConfig = stages.find(
      (stageEntry: any) =>
        stageEntry.stage == this.stage &&
        pickStageYear(stageEntry) === targetYear,
    );
    ...
    const temporalDataInformationImplementation =
      await this._TemporalSaveService
        .getDataInformationImplementation()
        .pipe(take(1))
        .toPromise();

    const nombreImplementacion = this.tareaImplementada
      ? (temporalDataInformationImplementation?.nombreImplementacion?.trim() ||
        `Implementación ${this.stage}-${this.phase}`)
      : '';
    ...
    const objLog = {
      ...
      NOMBRE_IMPLEMENTACION: nombreImplementacion,
    ...
    */
    ...
    try {
      const result: any = await (this.isCompletePhase
        ? this._UpdateStagePhaseCommand.execute(objLog)
        : this._SaveStagePhaseCommand.execute(objLog));
      if (result) {
        stages.map((stageEntry: any) => {
          if (
            stageEntry.stage == this.stage &&
            pickStageYear(stageEntry) === stageYear
          ) {
            stageEntry.survey.map((survey: any) => {
                ...
                const nextSurvey = stages
                  .find(
                    (entry: any) =>
                      ...
                      pickStageYear(entry) === stageYear,
                  )
                  ?.survey.find(
                    (surveyEntry: any) =>
                      surveyEntry.survey == `${this.stage}-${this.phase + 1}`,
                  );
      ...
      console.error('Error al guardar los datos primero:', error);
      Swal.fire({
  ...
  public async finishSurvey(step: number) {
    const temporalData = await this.validateAndSaveCurrentViewChild(
      step,
      this.tareaImplementada
        ? this.commentsComponent
        : this.undoneTaskComponent,
    );
  ...
  async validateDataInformationImplementation() {
    try {
      ...
      console.error(
        'Error al validar la información de implementación:',
        error,
      );
      return false;
    }
    ...
    try {
      ...
      if (dataUser) {
        ...
        if (stage) {
          ...
          let result = await (dataUser?.REGISTRADO
            ? this._UpdateConfigurationCommand.execute(objSave)
            : this._SaveConfigurationCommand.execute(objSave));
          if (result) {
            console.log(
  ...
  async getDataSaved() {
    ...
    try {
      ...
      const { NUMERO_DOCUMENTO: numeroDocumento, CODIGO_LOCAL: codigoLocal } =
        JSON.parse(dataUser);
      const stages = JSON.parse(savedStage);
      ...
      const currentStage = stages.find(
        (s: any) => s.stage === this.stage && pickStageYear(s) === targetYear,
      );
      if (!currentStage) return;

      const currentSurvey = currentStage.survey.find(
        (survey: any) => survey.survey === `${this.stage}-${this.phase}`,
      );
      ...
      dataResp = {
        ...
        ARCHIVO_EVIDENCIA_NAME:
          currentSurvey.dataSaved?.ARCHIVO_EVIDENCIA ?? '',
      };
      ...
      if ((error as any)?.message) {
        console.error(
          'Error al obtener los datos guardados:',
  ...
  fillQuestionAnswer(data: any, respuesta: any) {
    ...
    let objImplementacion: any = {
      ...
      nombreImplementacion: data.NOMBRE_IMPLEMENTACION,
      tareaImplementada: data.REGISTRA_TAREA,
    };
    this.tareaImplementada = data.REGISTRA_TAREA;
    this.question = data.REGISTRA_TAREA ? 1 : 5;
    ...
    if (data.REGISTRA_TAREA) {
      this.archivoName = {
        archivoActa: {
          name:
            data.ARCHIVO_ACTA && !data.ARCHIVO_ACTA_NAME
              ? 'acta.pdf'
              : data.ARCHIVO_ACTA_NAME,
          file: data.ARCHIVO_ACTA,
        },
        archivoFoto: {
          name:
            data.ARCHIVO_EVIDENCIA && !data.ARCHIVO_EVIDENCIA_NAME
              ? 'evidencia.jpg'
              : data.ARCHIVO_EVIDENCIA_NAME,
