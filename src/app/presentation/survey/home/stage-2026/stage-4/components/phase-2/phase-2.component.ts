...
@Component({
  ...
  styleUrl: './phase-2.component.css',
})
export class Phase2Component {
  ...
  private async validateAndSaveCurrentViewChild(
    ...
    // En esta fase no se requiere nombre de implementación
  ...
  private async saveSurveyData(temporalData: any): Promise<boolean> {
    ...
    const currentStageConfig = stages.find(
      (stageEntry: any) =>
        stageEntry.stage == this.stage && pickStageYear(stageEntry) === targetYear,
    );
    ...
    const objLog = {
      ...
      NOMBRE_IMPLEMENTACION: this.tareaImplementada
        ? temporalDataInformationImplementation?.nombreImplementacion ?? ''
        : '',
            ...
            stageEntry.survey.map((survey: any) => {
              if (survey.survey == `${this.stage}-${this.phase}`) {
                ...
                const nextSurvey = stages
                  .find(
                    (entry: any) =>
                      entry.stage == this.stage && pickStageYear(entry) === stageYear,
                  )
                  ?.survey.find(
                    (surveyEntry: any) =>
  ...
  private ensureActiveStageYear(): '2026' {
    const enforcedYear: '2026' = this.year;
