...
@Component({
  ...
  imports: [FormsModule, CommonModule, RadioButton, MultiSelectModule, InputNumberModule, DatePickerModule, SelectModule, InputNumberModule],
...
export class Quest2Component {
  ...
  // Control 6
  casosSiseve: { [key: number]: number } = { 2022: 0, 2023: 0, 2024: 0, 2025: 0 };
  ...
  constructor() {
    ...
    const contenidos = [
      { name: 'Sensibilización sobre disciplina desde el enfoque de derechos', code: 'CONT1' },
      { name: 'Fortalecimiento de las normas de convivencia del aula', code: 'CONT2' },
      { name: 'Aplicación de medidas correctivas desde el enfoque de derechos', code: 'CONT3' },
      { name: 'Estrategias de autocuidado y bienestar colectivo', code: 'CONT4' },
      { name: 'Mentalidades transformadoras', code: 'CONT5' },
      { name: 'Mediación y resolución de conflictos', code: 'CONT6' },
      { name: 'Identificación de riesgos en entornos físicos y virtuales', code: 'CONT7' },
      { name: 'Cultura del reporte de casos de violencia escolar', code: 'CONT8' },
      { name: 'Uso del Portal SíseVE', code: 'CONT9' },
      { name: 'Identificación de espacios inseguros', code: 'CONT10' },
      { name: 'Identificación de riesgos de bullying y discriminación étnico racial', code: 'CONT11' },
    ...
    this.resultadosPrevencion = [
      { name: 'Mejora el trato de docentes con las y los estudiantes.', code: 'RES1' },
      { name: 'Incremento del interés y participación de docentes en acciones del PREVI', code: 'RES2' },
      { name: 'Estudiantes confían en docentes que muestran empatía e interés por su bienestar.', code: 'RES3' },
      { name: 'Docentes que en hora de tutoría tratan temas para prevención de la violencia.', code: 'RES4' },
      { name: 'Mejora de las relaciones interpersonales entre integrantes de la comunidad educativa.', code: 'RES5' },
      { name: 'Se implementan acciones y/o sesiones para el fortalecimiento de la Educación Sexual Integral-ESI, a nivel de la IE y del aula.', code: 'RES6' },
      { name: 'Más personas de la escuela conocen el SíseVe para reportar hechos de violencia.', code: 'RES7' },
      { name: 'Se atienden los casos de violencia escolar y logra una escuela segura para estudiantes.', code: 'RES8' },
      { name: 'Se han promovido acciones para la prevención de la violencia escolar, desde los espacios de participación estudiantil', code: 'RES9' },
      { name: 'Mayor involucramiento de las familias en las acciones de la BAPE.', code: 'RES10' },
    ];
  }
  ...
  ngOnInit() {
    if (this.dataEntry) {
      ...
      this.selectedContenidosAcogidos = (this.dataEntry.contenidosAcogidos || '').split(',').filter((x: string) => x);
      this.selectedContenidosDesafio = (this.dataEntry.contenidosDesafio || '').split(',').filter((x: string) => x);
      this.selectedResultadosPrevencion = (this.dataEntry.resultadosPrevencion || '').split(',').filter((x: string) => x);
