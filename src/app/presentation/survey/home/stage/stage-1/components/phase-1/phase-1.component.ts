...
@Component({
  ...
  styleUrl: './phase-1.component.css',
})
...
showUndoneTask(): boolean {
  ...
  private async saveSurveyData(temporalData: any): Promise<boolean> {
    ...
    const stageYear = currentStageConfig?.YEAR ?? currentStageConfig?.year ?? '2026';
    ...
    const archivoActaBase64 = this.archivoActa
      ? await this.convertFileToBase64(this.archivoActa)
      : null;
    const archivoFotoBase64 = this.archivoFoto
      ? await this.convertFileToBase64(this.archivoFoto)
      : null;
